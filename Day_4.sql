...
SELECT * FROM insanlar2
WHERE soyisim='Bulut'
ORDER BY 4; -- Sıralamada field numarası da kullanabiliriz ama burda 1 2 3 diye gidiyor 1 e karşılık ssn mesela

/*
NOT:Ascending (ASC) doğal sıralama(NATURAL) küçükten büyüğe, yazılmasada defauld olarak natural sıralama yapar
...
-- Sirketlere göre maaşı 5000 liradan fazla olan personel sayısını bulun
SELECT sirket , COUNT(*) FROM personel5
WHERE maas>5000
GROUP BY sirket;
